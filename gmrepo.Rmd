...
ancom_6070 <- ancombc2(data = ps_6070, assay_name = "counts",
                       ...
                       trend_control = list(contrast = list(matrix(c(1, 0, -1, 1), # nolint
                                                                   nrow = 2,
                                                                   byrow = TRUE), # nolint
                                                             matrix(c(-1, 0, 1, -1), # nolint
                                                                    nrow = 2,
                                                                    byrow = TRUE), # nolint
                                                             matrix(c(1, 0, 1, -1), # nolint
                                                                    nrow = 2,
...
# Ancombc2
ancom_7928 <- ancombc2(data = ps_7928, assay_name = "counts",
                       tax_level = "Species",
                       fix_formula = "associated.phenotype",
                       rand_formula = NULL,
                       p_adj_method = "fdr", pseudo_sens = TRUE,
...
plot_cclasso <- function(cor_map, adj_mat, phyloseq) {
  ...
  #gd <- set.vertex.attribute(gd, "degree", index = V(gd)$otu, value = degall)  # nolint
  ...
  pl <- plot(gd,
    ...
    #vertex.size=(log10(V(gd)$size))^1.5/2, # nolint: commented_code_linter.
    vertex.size = 5,
    vertex.label.dist = 0.75, vertex.label.color = "black",
    vertex.label.font = 3,
    edge.arrow.size = 2, asp = TRUE,
    #edge.width = abs(E(gd)$cor)*7, # nolint: commented_code_linter.
    edge.width = 4,
...
#Exploration
plot(density(as.numeric(net_6070_cormap)),
     main = "All interactions") ## most interaction has strength centering zero
plot(density(as.numeric(net_6070_cormap[abs(net_6070_cormap) > 0.5])),
     main = "filtered")
