...
plot_cclasso <- function(cor.map, adj.matrix, phyloseq) {
  ...
  feat_annot$size <- as.vector(taxa_sums(phyloseq)[as.numeric(all.nodes)]) # abundance
  feat_annot$Family <- as.vector(tax_table(phyloseq)[as.numeric(all.nodes),"Family"]) #Assign taxonomy of the Family level
